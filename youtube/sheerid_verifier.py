"""Program utama verifikasi pelajar SheerID"""
import re
import random
import logging
import httpx
...
class SheerIDVerifier:
    """Verifier identitas pelajar SheerID"""
    ...
    def normalize_url(url: str) -> str:
        """Normalisasi URL (dibiarkan apa adanya)"""
        return url
    ...
    def _sheerid_request(
        ...
        try:
            ...
            logger.error(f"Permintaan SheerID gagal: {e}")
            raise
    ...
    def verify(
        ...
        """Jalankan alur verifikasi tanpa polling status agar lebih cepat"""
        try:
            ...
            logger.info(f"Info pelajar: {first_name} {last_name}")
            ...
            # Buat PNG kartu pelajar
            logger.info("Langkah 1/4: buat PNG kartu pelajar...")
            img_data = generate_image(first_name, last_name, school_id)
            file_size = len(img_data)
            logger.info(f"✅ Ukuran PNG: {file_size / 1024:.2f}KB")

            # Kirim info pelajar
            logger.info("Langkah 2/4: kirim info pelajar...")
            step2_body = {
            ...
            if step2_data.get("currentStep") == "error":
                ...
                raise Exception(f"Kesalahan langkah 2: {error_msg}")
            ...
            if current_step in ["sso", "collectStudentPersonalInfo"]:
                logger.info("Langkah 3/4: lewati verifikasi SSO...")
            ...
            # Unggah dokumen lalu selesaikan
            logger.info("Langkah 4/4: minta dan unggah dokumen...")
            step4_body = {
                "files": [
            ...
            if not step4_data.get("documents"):
                raise Exception("Tidak bisa mendapatkan URL unggah")

            upload_url = step4_data["documents"][0]["uploadUrl"]
            logger.info("✅ Berhasil mendapatkan URL unggah")
            if not self._upload_to_s3(upload_url, img_data):
                raise Exception("Gagal mengunggah ke S3")
            logger.info("✅ Kartu pelajar berhasil diunggah")

            step6_data, _ = self._sheerid_request(
                "POST",
                f"{config.SHEERID_BASE_URL}/rest/v2/verification/{self.verification_id}/step/completeDocUpload",
            )
            logger.info(f"✅ Pengiriman dokumen selesai: {step6_data.get('currentStep')}")
            final_status = step6_data

            # Tanpa polling status; langsung menunggu peninjauan
            return {
                "success": True,
                "pending": True,
...
def main():
    ...
    print("=" * 60)
    print("Alat verifikasi identitas pelajar SheerID (versi Python)")
    print("=" * 60)
    print()
    ...
    if not verification_id:
        print("❌ Kesalahan: format ID verifikasi tidak valid")
        sys.exit(1)

    print(f"✅ Berhasil membaca ID verifikasi: {verification_id}")
    print()
