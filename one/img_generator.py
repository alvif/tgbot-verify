"""Modul generator PNG kartu pelajar - Penn State LionPATH"""
import random
...
<head>
    ...
    <style>
        ...
        body {{
            ...
            background-color: #e0e0e0; /* Latar belakang browser */
            margin: 0;
            padding: 20px;
...
def generate_image(first_name, last_name, school_id='2565'):
    """
    Buat tangkapan PNG Penn State LionPATH
    ...
    try:
        ...
        # Gunakan Playwright untuk mengambil tangkapan layar (pengganti Selenium)
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            page = browser.new_page(viewport={'width': 1200, 'height': 900})
            page.set_content(html_content, wait_until='load')
            page.wait_for_timeout(500)  # Tunggu gaya terpasang
...
if __name__ == '__main__':
    ...
    # Perbaiki masalah encoding konsol Windows
    if sys.platform == 'win32':
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

    print("Menguji pembuatan gambar PSU...")
