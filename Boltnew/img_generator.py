"""Modul generator PNG bukti guru - Bolt.now / PSU"""
import random
from datetime import datetime


def generate_psu_id():
    """Buat ID PSU acak (9 digit)"""
...
def generate_psu_email(first_name, last_name):
    """
    Buat email PSU
    Format: firstName.lastName + 3-4 angka @psu.edu
    """
...
def _get_browser_context():
    """Ambil atau buat konteks browser (mode tunggal)"""
    ...
    if _browser_context is None:
        try:
            ...
            raise Exception("Playwright belum terpasang: jalankan `pip install playwright` lalu `playwright install chromium`")
    return _browser_context


def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    """Ubah HTML menjadi tangkapan PNG (optimasi: pakai ulang instance browser)"""
    try:
        context = _get_browser_context()
        page = context.new_page()

        try:
            # Setel konten HTML langsung dan gunakan domcontentloaded alih-alih networkidle (lebih cepat)
            page.set_content(html_content, wait_until='domcontentloaded')

            # Tunggu gambar termuat (jika ada gambar eksternal)
            page.wait_for_load_state('load', timeout=3000)

            # Hitung tinggi secara otomatis
            if height is None:
...
def generate_employment_letter_html(
    ...
    """Bangun HTML surat keterangan kerja guru."""
    name = f"{first_name} {last_name}"
    now = datetime.now()
...
def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    """
    Hasilkan banyak PNG secara paralel (optimasi performa)

    Args:
        html_list: [(html_content, width, height), ...]

    Returns:
        list[bytes]: daftar data PNG
    """
    import asyncio
    from playwright.async_api import async_playwright

    async def render_single(html_content: str, width: int, height: int):
        """Render satu gambar secara asinkron"""
    ...
    async def render_all():
        """Render semua gambar secara paralel"""
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Hasilkan dua PNG: kartu guru + surat kerja (optimasi paralel)

    Args:
        first_name: nama depan
        last_name: nama belakang
        school_id: ID sekolah (dipertahankan agar kompatibel)
    ...
    # Hasilkan dua gambar secara paralel
    html_list = [
        (card_html, 700, 1100),
