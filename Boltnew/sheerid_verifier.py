"""Program utama verifikasi guru SheerID (Bolt.now)"""
import re
import random
import logging
import httpx
...
class SheerIDVerifier:
    ...
    def normalize_url(url: str) -> str:
        """Normalisasi URL (dibiarkan apa adanya demi kompatibilitas)"""
        return url
    ...
    def create_verification(self) -> str:
        """Ajukan verificationId baru lewat installPageUrl"""
        body = {
        ...
        if status != 200 or not isinstance(data, dict) or not data.get("verificationId"):
            raise Exception(f"Gagal membuat verification (kode status {status}): {data}")

        self.verification_id = data["verificationId"]
        logger.info(f"✅ Mendapat verificationId: {self.verification_id}")
        return self.verification_id

    def _sheerid_request(
        self, method: str, url: str, body: Optional[Dict] = None
    ) -> Tuple[Dict, int]:
        """Kirim permintaan API SheerID"""
        headers = {
    ...
    def _upload_to_s3(self, upload_url: str, img_data: bytes) -> bool:
        ...
        try:
            ...
            logger.error(f"Gagal mengunggah ke S3: {e}")
            return False
    ...
    def verify(
        ...
        try:
            ...
            if not self.verification_id:
                logger.info("Mengajukan verificationId baru ...")
                self.create_verification()

            logger.info(f"Info guru: {first_name} {last_name}")
            ...
            # Buat PNG guru
            logger.info("Langkah 1/5: membuat dokumen PNG guru...")
            assets = generate_images(first_name, last_name, school_id)
            for asset in assets:
                logger.info(
                    f"  - {asset['file_name']} ukuran: {len(asset['data'])/1024:.2f}KB"
                )

            # Kirim info guru
            logger.info("Langkah 2/5: mengirim info guru...")
            step2_body = {
            ...
            if isinstance(step2_data, dict) and step2_data.get("currentStep") == "error":
                ...
                raise Exception(f"Kesalahan pada langkah 2: {error_msg}")

            logger.info(f"✅ Langkah 2 selesai: {getattr(step2_data, 'get', lambda k, d=None: d)('currentStep')}")
            current_step = (
            ...
            if current_step in ["sso", "collectTeacherPersonalInfo"]:
                ...
                logger.info(f"✅ Langkah 3 selesai: {getattr(step3_data, 'get', lambda k, d=None: d)('currentStep')}")
                current_step = (
                    step3_data.get("currentStep", current_step) if isinstance(step3_data, dict) else current_step
                )

            # Minta URL unggah dan unggah dokumen
            logger.info("Langkah 4/5: meminta URL unggah ...")
            step4_body = {
                "files": [
                    {
            ...
            if step4_status != 200 or not isinstance(step4_data, dict) or not step4_data.get("documents"):
                raise Exception(f"Tidak bisa mendapatkan URL unggah: {step4_data}")

            documents = step4_data["documents"]
            if len(documents) != len(assets):
                raise Exception("Jumlah tugas unggah tidak cocok dengan jumlah file")

            for doc, asset in zip(documents, assets):
                upload_url = doc.get("uploadUrl")
                if not upload_url:
                    raise Exception("URL unggah tidak tersedia")
                if not self._upload_to_s3(upload_url, asset["data"]):
                    raise Exception(f"Gagal mengunggah ke S3: {asset['file_name']}")
                logger.info(f"✅ Berhasil mengunggah {asset['file_name']}")

            step6_data, _ = self._sheerid_request(
                "POST",
                f"{config.SHEERID_BASE_URL}/rest/v2/verification/{self.verification_id}/step/completeDocUpload",
            )
            logger.info(f"✅ Pengiriman dokumen selesai: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")

            # Ambil status akhir (termasuk rewardCode)
            ...
            return {
                ...
                "message": "Dokumen telah dikirim, menunggu peninjauan"
...
def main():
    """Fungsi utama - antarmuka command line"""
    import sys

    print("=" * 60)
    print("Alat verifikasi identitas guru SheerID (versi Python)")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Masukkan tautan verifikasi SheerID (termasuk externalUserId): ").strip()

    if not url:
        print("❌ Kesalahan: URL tidak disediakan")
        sys.exit(1)

    verification_id = SheerIDVerifier.parse_verification_id(url)
    verifier = SheerIDVerifier(url, verification_id=verification_id)

    print(f"👉 Menggunakan tautan: {verifier.install_page_url}")
    if verifier.verification_id:
        print(f"Berhasil mengurai verificationId: {verifier.verification_id}")
    ...
    if result.get("reward_code"):
        print(f"Kode promo: {result['reward_code']}")
